...
// Send "batch check started" notification to user
try {
    if (function_exists('sendTelegramHtml') && SiteConfig::get('notify_check_started', true)) {
        $batch_start_notification = (
            "🔄 <b>Batch Card Check Started</b>\n\n" .
            "👤 <b>User ID:</b> {$telegram_id}\n" .
            "📦 <b>Total Cards:</b> " . count($valid_cards) . "\n" .
            "🔗 <b>Sites:</b> " . count($valid_sites) . "\n" .
            "⚡ <b>Concurrent:</b> {$max_concurrent}\n" .
            "⏳ <b>Status:</b> Checking in progress..."
        );
        sendTelegramHtml($batch_start_notification);
    }
} catch (Exception $e) {
    error_log("Failed to send batch check started notification: " . $e->getMessage());
}
