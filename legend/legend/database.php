...
class Database {
    ...
    public function getUserProxies($telegramId) {
        if ($this->useFallback) {
            return $this->getFallback()->getUserProxies($telegramId);
        }

        $collection = $this->getCollection(DatabaseConfig::USER_PROXIES_COLLECTION);
    ...
    public function addUserProxy($telegramId, array $proxyData) {
        if ($this->useFallback) {
            return $this->getFallback()->addUserProxy($telegramId, $proxyData);
        }
        ...
        $setData = [
            ...
            'last_checked_at' => isset($proxyData['last_checked_at'])
                ? new MongoDB\BSON\UTCDateTime($proxyData['last_checked_at'])
                : null,
        ...
        $result = $collection->updateOne(
            ['user_id' => $telegramId, 'proxy' => $proxyData['proxy']],
        ...
        return $existing && isset($existing->_id) ? (string) $existing->_id : null;
    }

    public function addUserProxies($telegramId, array $proxies) {
        $ids = [];
        foreach ($proxies as $proxyData) {
            $id = $this->addUserProxy($telegramId, $proxyData);
            if ($id !== null) {
                $ids[] = $id;
            }
        }
    ...
    public function removeUserProxies($telegramId, array $proxyIds) {
        if ($this->useFallback) {
            return $this->getFallback()->removeUserProxies($telegramId, $proxyIds);
        }
    ...
    public function updateUserProxyStatus($telegramId, $proxyId, array $statusData) {
        if ($this->useFallback) {
            return $this->getFallback()->updateUserProxyStatus($telegramId, $proxyId, $statusData);
        }

        try {
            $objectId = new MongoDB\BSON\ObjectId($proxyId);
        } catch (Exception $e) {
            return false;
        }
        ...
        $collection = $this->getCollection(DatabaseConfig::USER_PROXIES_COLLECTION);
        $update = [
            '$set' => array_merge($statusData, [
                'updated_at' => new MongoDB\BSON\UTCDateTime(),
            ]),
        ];

        if (isset($statusData['last_checked_at']) && is_int($statusData['last_checked_at'])) {
            $update['$set']['last_checked_at'] = new MongoDB\BSON\UTCDateTime($statusData['last_checked_at']);
        }

        $result = $collection->updateOne(
            ['_id' => $objectId, 'user_id' => $telegramId],
            $update
        );
