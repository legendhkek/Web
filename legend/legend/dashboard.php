...
<body>
    ...
    <div class="drawer">
        ...
        <div class="drawer-menu">
            ...
            <div id="proxySection" style="display:none; background: var(--bg-secondary); margin: 0 16px; border-radius: 12px; padding: 16px;">
                <div class="proxy-controls" style="display:flex; flex-direction:column; gap: 12px;">
                    <textarea id="proxyBulkInput" placeholder="Enter proxies (one per line). Formats supported: host:port or host:port:user:pass" style="width:100%; min-height:90px; padding:10px; border:1px solid var(--border-color); border-radius:8px; background:var(--bg-card); color:var(--text-primary); font-size:12px; resize:vertical;"></textarea>
                    <div class="proxy-actions" style="display:flex; flex-wrap:wrap; gap:8px;">
                        <button id="proxyAddBtn" style="flex:1; min-width:140px; padding:10px 12px; background:var(--accent-green); color:white; border:none; border-radius:8px; font-size:12px; cursor:pointer;">Add Proxies</button>
                        <button id="proxyAddTestBtn" style="flex:1; min-width:140px; padding:10px 12px; background:var(--accent-blue); color:white; border:none; border-radius:8px; font-size:12px; cursor:pointer;">Add &amp; Test</button>
                        <button id="proxyRetestBtn" style="flex:1; min-width:140px; padding:10px 12px; background:var(--accent-orange); color:white; border:none; border-radius:8px; font-size:12px; cursor:pointer;">Retest Selected</button>
                        <button id="proxyDeleteBtn" style="flex:1; min-width:140px; padding:10px 12px; background:var(--error-color); color:white; border:none; border-radius:8px; font-size:12px; cursor:pointer;">Remove Selected</button>
                    </div>
                    <div id="proxyStatus" style="font-size:12px; min-height:18px; color:var(--text-secondary);"></div>
                    <div class="proxy-table-wrapper" style="max-height:220px; overflow:auto; border:1px solid var(--border-color); border-radius:10px;">
                        <table style="width:100%; border-collapse:collapse; font-size:12px;">
                            <thead>
                                <tr style="background:var(--bg-card); color:var(--text-secondary);">
                                    <th style="padding:8px; text-align:left; width:32px;"><input type="checkbox" id="selectAllProxies"></th>
                                    <th style="padding:8px; text-align:left;">Proxy</th>
                                    <th style="padding:8px; text-align:left; width:90px;">Status</th>
                                    <th style="padding:8px; text-align:left; width:80px;">Country</th>
                                    <th style="padding:8px; text-align:left; width:80px;">Latency</th>
                                    <th style="padding:8px; text-align:left; width:140px;">Last Checked</th>
                                    <th style="padding:8px; text-align:left; width:80px;">Actions</th>
                                </tr>
                            </thead>
                            <tbody id="proxyTableBody">
                                <tr>
                                    <td colspan="7" style="padding:16px; text-align:center; color:var(--text-secondary);">No proxies added yet.</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    <script nonce="<?php echo $nonce; ?>">
        ...
        let proxyStatusTimeout = null;

        function setProxyStatus(message, type = 'info', persist = false) {
            ...
            statusEl.style.color = colors[type] || 'var(--text-secondary)';

            if (proxyStatusTimeout) {
                clearTimeout(proxyStatusTimeout);
                proxyStatusTimeout = null;
            }

            if (!persist && message) {
                proxyStatusTimeout = setTimeout(() => {
        ...
        function normalizeProxy(proxy) {
            return {
                id: proxy.id,
                proxy: proxy.proxy,
                masked_proxy: proxy.masked_proxy || ProxyServiceMask(proxy.proxy),
                status: (proxy.status || 'unknown').toLowerCase(),
                country: proxy.country || '—',
                latency_ms: typeof proxy.latency_ms === 'number' ? proxy.latency_ms : null,
        ...
        function ProxyServiceMask(proxyString) {
        ...
        function formatStatusBadge(status) {
            const baseStyles = 'display:inline-flex;align-items:center;justify-content:center;padding:2px 8px;border-radius:999px;font-size:10px;font-weight:600;text-transform:uppercase;';
            const colors = {
                live: { bg: 'rgba(34,197,94,0.15)', color: '#22c55e' },
                charged: { bg: 'rgba(34,197,94,0.15)', color: '#22c55e' },
                approved: { bg: 'rgba(16,185,129,0.15)', color: '#10b981' },
                dead: { bg: 'rgba(239,68,68,0.15)', color: '#ef4444' },
                error: { bg: 'rgba(239,68,68,0.15)', color: '#ef4444' },
                unknown: { bg: 'rgba(148,163,184,0.15)', color: '#94a3b8' },
                testing: { bg: 'rgba(59,130,246,0.15)', color: '#3b82f6' }
            };
            const palette = colors[status] || colors.unknown;
            return `${baseStyles}background:${palette.bg};color:${palette.color};`;
        }
        ...
        function updateSelectAllCheckbox() {
            ...
            const selectedCount = proxyState.selected.size;
            selectAll.checked = selectedCount === proxyState.proxies.length;
            selectAll.indeterminate = selectedCount > 0 && selectedCount < proxyState.proxies.length;
        }
        ...
        function renderProxyTable() {
            ...
            if (!proxyState.proxies.length) {
                ...
                cell.textContent = 'No proxies added yet.';
            ...
            proxyState.proxies.forEach(proxy => {
                ...
                row.dataset.proxyId = normalized.id;
                row.style.borderBottom = `1px solid var(--border-color)`;

                // Select checkbox
                const selectCell = document.createElement('td');
                selectCell.style.padding = '8px';
                selectCell.style.width = '32px';
                const checkbox = document.createElement('input');
                checkbox.type = 'checkbox';
                checkbox.checked = proxyState.selected.has(normalized.id);
                checkbox.addEventListener('change', () => {
                    if (checkbox.checked) {
                        proxyState.selected.add(normalized.id);
                    } else {
                        proxyState.selected.delete(normalized.id);
                    }
                    updateSelectAllCheckbox();
                });
                ...
                proxyCell.style.padding = '8px';
                proxyCell.style.color = 'var(--text-primary)';

                const proxyText = document.createElement('div');
                proxyText.style.fontFamily = 'monospace';
                proxyText.style.fontSize = '11px';
                proxyText.textContent = normalized.masked_proxy;

                const hint = document.createElement('div');
                hint.style.fontSize = '10px';
                hint.style.color = 'var(--text-secondary)';
                hint.textContent = normalized.label ? normalized.label : normalized.proxy;

                proxyCell.appendChild(proxyText);
                proxyCell.appendChild(hint);
                ...
                badge.setAttribute('style', formatStatusBadge(normalized.status));
                badge.textContent = normalized.status;
                ...
                countryCell.textContent = normalized.country || '—';
                row.appendChild(countryCell);

                // Latency
                const latencyCell = document.createElement('td');
                latencyCell.style.padding = '8px';
                latencyCell.textContent = normalized.latency_ms ? `${normalized.latency_ms} ms` : '—';
                row.appendChild(latencyCell);

                // Last checked
                const checkedCell = document.createElement('td');
                checkedCell.style.padding = '8px';
                checkedCell.textContent = normalized.last_checked_at ? formatDateDisplay(normalized.last_checked_at) : '—';
                ...
                testBtn.style.cssText = 'background:var(--accent-blue);border:none;color:#fff;padding:4px 8px;border-radius:6px;font-size:11px;cursor:pointer;margin-right:6px;';
                testBtn.addEventListener('click', () => testProxy(normalized.id));

                const copyBtn = document.createElement('button');
                copyBtn.textContent = 'Copy';
                copyBtn.style.cssText = 'background:var(--accent-green);border:none;color:#fff;padding:4px 8px;border-radius:6px;font-size:11px;cursor:pointer;';
                copyBtn.addEventListener('click', async () => {
                    try {
                        await navigator.clipboard.writeText(normalized.proxy);
                        setProxyStatus('Proxy copied to clipboard', 'success');
                    } catch (err) {
                        setProxyStatus('Clipboard copy failed', 'error');
                    }
                });
        ...
        async function loadProxies(forceReload = false) {
            if (proxyState.loading) return;
            if (!forceReload && proxyState.proxies.length && proxyState.initialized) {
                renderProxyTable();
                return;
            }

            proxyState.loading = true;
            setProxyStatus('Loading proxies...', 'info', true);
            try {
                ...
                proxyState.proxies = data.proxies || [];
                const available = new Set(proxyState.proxies.map(p => p.id));
                proxyState.selected = new Set([...proxyState.selected].filter(id => available.has(id)));
                renderProxyTable();
                setProxyStatus(`Loaded ${proxyState.proxies.length} proxies`, 'success');
            } catch (error) {
                console.error('Proxy load failed', error);
                setProxyStatus('Failed to load proxies', 'error');
            } finally {
        ...
        async function addProxies(autoTest = false) {
            const textarea = document.getElementById('proxyBulkInput');
            if (!textarea) return;
            const rawValue = textarea.value.trim();
            if (!rawValue) {
                setProxyStatus('Enter at least one proxy', 'warning');
                return;
            }

            setProxyStatus(autoTest ? 'Adding and testing proxies...' : 'Adding proxies...', 'info', true);
            ...
            try {
                ...
                let message = `Added ${addedCount} proxies`;
                if (autoTest) {
                    message += ' (tested)';
                }
                if (errorCount) {
                ...
                setProxyStatus(message, errorCount ? 'warning' : 'success');
                await loadProxies(true);
            } catch (error) {
                console.error('Failed to add proxies', error);
                setProxyStatus(error.message || 'Failed to add proxies', 'error');
            } finally {
        ...
        async function deleteSelectedProxies() {
            if (!proxyState.selected.size) {
                setProxyStatus('Select proxies to remove', 'warning');
                return;
            }

            const ids = Array.from(proxyState.selected);
            setProxyStatus(`Removing ${ids.length} proxies...`, 'info', true);
            disableProxyButtons(true);
            ...
            try {
                ...
                setProxyStatus(`Removed ${data.deleted} proxies`, 'success');
                await loadProxies(true);
            } catch (error) {
                console.error('Failed to remove proxies', error);
                setProxyStatus(error.message || 'Failed to remove proxies', 'error');
            } finally {
                disableProxyButtons(false);
            }
        }
        ...
        async function testProxy(id) {
            if (!id) return;
            setProxyStatus('Testing proxy...', 'info', true);
            ...
            try {
                ...
                setProxyStatus(`Proxy is ${status.toUpperCase()}`, status === 'live' ? 'success' : (status === 'dead' ? 'error' : 'info'));
                await loadProxies(true);
            } catch (error) {
        ...
        async function retestSelectedProxies() {
            if (!proxyState.selected.size) {
                setProxyStatus('Select proxies to retest', 'warning');
                return;
            }

            const ids = Array.from(proxyState.selected);
            setProxyStatus(`Retesting ${ids.length} proxies...`, 'info', true);
            disableProxyButtons(true);
            ...
            try {
                ...
                setProxyStatus('Retest complete', 'success');
            } finally {
                disableProxyButtons(false);
            }
        }
        ...
        function disableProxyButtons(disabled) {
            const buttons = [
                document.getElementById('proxyAddBtn'),
                document.getElementById('proxyAddTestBtn'),
                document.getElementById('proxyRetestBtn'),
                document.getElementById('proxyDeleteBtn')
            ];
            buttons.forEach(btn => {
        ...
        function initProxyManager() {
            ...
            const addTestBtn = document.getElementById('proxyAddTestBtn');
            const deleteBtn = document.getElementById('proxyDeleteBtn');
            const retestBtn = document.getElementById('proxyRetestBtn');
            const selectAll = document.getElementById('selectAllProxies');

            if (addBtn) addBtn.addEventListener('click', () => addProxies(false));
            if (addTestBtn) addTestBtn.addEventListener('click', () => addProxies(true));
            if (deleteBtn) deleteBtn.addEventListener('click', deleteSelectedProxies);
            if (retestBtn) retestBtn.addEventListener('click', retestSelectedProxies);
            if (selectAll) {
                selectAll.addEventListener('change', () => {
                    if (selectAll.checked) {
                        proxyState.proxies.forEach(proxy => proxyState.selected.add(proxy.id));
                    } else {
                        proxyState.selected.clear();
                    }
                    renderProxyTable();
        ...
        function getProxyPool() {
            return proxyState.proxies.map(proxy => normalizeProxy(proxy).proxy);
        }

        function getNextManagedProxy() {
            const pool = getProxyPool();
            if (!pool.length) return null;
            const proxy = pool[proxyState.rotationIndex % pool.length];
            proxyState.rotationIndex = (proxyState.rotationIndex + 1) % pool.length;
            return proxy;
        }
        ...
        function toggleProxySection() {
            ...
            const shouldOpen = section.style.display === 'none' || section.style.display === '';
        ...
        // Make selective functions globally available
        window.toggleQuickTools = toggleQuickTools;
        window.toggleProxySection = toggleProxySection;
        window.getNextProxy = getNextManagedProxy;
        window.getProxyPool = getProxyPool;
    </script>
    <script nonce="<?php echo $nonce; ?>">
