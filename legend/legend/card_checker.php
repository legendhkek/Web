...
$currentCredits = isset($_SESSION['credits']) ? (int)$_SESSION['credits'] : null;
?>
    ...
    <div class="main-wrapper">
        <div class="panel">
            ...
            <form id="checkForm">
                ...
                <div class="saved-proxy-panel" style="margin-top: 15px; padding: 15px; border:1px solid rgba(0,255,234,0.2); border-radius:10px; background:rgba(0,0,0,0.25);">
                    <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                        <div style="font-weight:700; color:var(--text-color); display:flex; align-items:center; gap:8px;">
                            <i class="fas fa-network-wired"></i> Saved Proxies
                        </div>
                        <div style="display:flex; gap:8px; flex-wrap:wrap;">
                            <button type="button" id="refreshSavedProxiesBtn" style="background:var(--link-color); color:#fff; border:none; border-radius:6px; padding:6px 12px; font-size:0.85em; cursor:pointer;">Refresh</button>
                            <a href="dashboard.php#proxy" target="_blank" style="background:var(--button-warning); color:#000; border:none; border-radius:6px; padding:6px 12px; font-size:0.85em; text-decoration:none; display:inline-flex; align-items:center; gap:6px;">
                    ...
                    <div style="display:flex; flex-wrap:wrap; gap:15px; align-items:center; margin-top:12px;">
                        <label style="display:flex; align-items:center; gap:10px; font-size:0.9em;">
                            <input type="checkbox" id="useSavedProxiesToggle" />
                            <span>Use saved proxies (<span id="savedProxyCount">0</span> available)</span>
                        </label>
                        <div style="display:flex; align-items:center; gap:8px; font-size:0.85em;">
                            <span>Rotation:</span>
                            <select id="proxyRotationMode" style="background:var(--input-bg); color:var(--text-color); border:1px solid var(--border-color); border-radius:6px; padding:6px 10px;">
                                <option value="sequential">Sequential</option>
                                <option value="random">Random</option>
                            </select>
                        </div>
                        <div id="savedProxyHint" style="font-size:0.8em; color:var(--placeholder-color);">Proxy input below is ignored when saved proxies are in use.</div>
                    </div>
                </div>
                <textarea name="proxy_input" id="proxyInput" placeholder="Enter Proxy (format: host:port or host:port:user:pass) - Optional" rows="2"></textarea>
                <div class="input-group" style="margin-top:8px; align-items:center; gap:8px;">
                    <input type="checkbox" id="useMyIpCheckbox" />
                    <label for="useMyIpCheckbox" style="margin:0;">Use my IP (no proxy)</label>
                </div>
        ...
        <div class="panel stats-panel">
            ...
            <div style="text-align:center; margin-bottom:15px; font-size:0.95em; color:var(--text-color);">
                Remaining Credits: <span id="remainingCreditsDisplay"><?php echo $currentCredits !== null ? htmlspecialchars(number_format($currentCredits)) : '--'; ?></span>
            </div>
...
const useSavedProxiesToggle = document.getElementById('useSavedProxiesToggle');
const proxyRotationModeSelect = document.getElementById('proxyRotationMode');
const savedProxyCountLabel = document.getElementById('savedProxyCount');
const refreshSavedProxiesBtn = document.getElementById('refreshSavedProxiesBtn');
const savedProxyHint = document.getElementById('savedProxyHint');
const proxyInput = document.getElementById('proxyInput');
const remainingCreditsDisplay = document.getElementById('remainingCreditsDisplay');
...
async function fetchSavedProxies(showToast = false) {
    if (!savedProxyCountLabel) return;
    try {
        const response = await fetch('api/proxies.php', { credentials: 'same-origin' });
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}`);
        }
        const data = await response.json();
        if (!data.success) {
            throw new Error(data.error || 'Failed to load saved proxies');
        }
        savedProxyState.proxies = (data.proxies || []).map(proxy => proxy.proxy).filter(Boolean);
        savedProxyState.pointer = 0;
        savedProxyCountLabel.textContent = savedProxyState.proxies.length;
        if (showToast) {
            alert(`Loaded ${savedProxyState.proxies.length} saved proxies.`);
        }
    } catch (error) {
        console.error('Failed to fetch saved proxies', error);
        if (showToast) {
            alert(error.message || 'Failed to load saved proxies');
        }
    } finally {
        updateSavedProxyUI();
    }
...
function updateSavedProxyUI() {
    if (savedProxyCountLabel) {
        savedProxyCountLabel.textContent = savedProxyState.proxies.length;
    }
    const usingSaved = !!(useSavedProxiesToggle && useSavedProxiesToggle.checked && !useMyIpCheckbox.checked);
    if (proxyRotationModeSelect) {
        proxyRotationModeSelect.disabled = !usingSaved;
    }
    if (savedProxyHint) {
        savedProxyHint.style.opacity = usingSaved ? 1 : 0.6;
    }
    if (proxyInput) {
        proxyInput.disabled = usingSaved;
        proxyInput.style.opacity = usingSaved ? 0.6 : 1;
    }
}

function syncProxyControls() {
    if (useMyIpCheckbox && useMyIpCheckbox.checked) {
        if (useSavedProxiesToggle) {
            useSavedProxiesToggle.checked = false;
            useSavedProxiesToggle.disabled = true;
        }
    } else if (useSavedProxiesToggle) {
        useSavedProxiesToggle.disabled = false;
    }
    if (proxyRotationModeSelect) {
        savedProxyState.mode = proxyRotationModeSelect.value;
    }
    updateSavedProxyUI();
}

function nextSavedProxy() {
    if (!savedProxyState.proxies.length) {
        throw new Error('NO_SAVED_PROXIES');
    }
    if (savedProxyState.mode === 'random') {
        const idx = Math.floor(Math.random() * savedProxyState.proxies.length);
        return savedProxyState.proxies[idx];
    }
    const proxy = savedProxyState.proxies[savedProxyState.pointer % savedProxyState.proxies.length];
    savedProxyState.pointer = (savedProxyState.pointer + 1) % savedProxyState.proxies.length;
    return proxy;
}

function determineProxyParams() {
    if (useMyIpCheckbox && useMyIpCheckbox.checked) {
        return { query: '&noproxy=1', proxy: null };
    }

    if (useSavedProxiesToggle && useSavedProxiesToggle.checked) {
        const proxy = nextSavedProxy();
        return { query: `&proxy=${encodeURIComponent(proxy)}`, proxy };
    }

    const manualProxy = proxyInput ? proxyInput.value.trim() : '';
    if (manualProxy) {
        return { query: `&proxy=${encodeURIComponent(manualProxy)}`, proxy: manualProxy };
    }

    return { query: '', proxy: null };
}

function updateRemainingCreditsDisplayValue(value) {
    if (!remainingCreditsDisplay) return;
    if (value === undefined || value === null || isNaN(value)) return;
    const numeric = Number(value);
    remainingCreditsDisplay.textContent = Number.isFinite(numeric) ? numeric.toLocaleString() : value;
}

if (refreshSavedProxiesBtn) {
    refreshSavedProxiesBtn.addEventListener('click', () => fetchSavedProxies(true));
}

if (useSavedProxiesToggle) {
    useSavedProxiesToggle.addEventListener('change', () => {
        if (useSavedProxiesToggle.checked && !savedProxyState.proxies.length) {
            alert('No saved proxies available. Refresh or add proxies first.');
            useSavedProxiesToggle.checked = false;
        } else {
            savedProxyState.pointer = 0;
        }
...
if (proxyRotationModeSelect) {
    proxyRotationModeSelect.addEventListener('change', () => {
        savedProxyState.mode = proxyRotationModeSelect.value;
    });
}

if (useMyIpCheckbox) {
    useMyIpCheckbox.addEventListener('change', syncProxyControls);
}

syncProxyControls();

fetchSavedProxies().finally(() => {
    updateSavedProxyUI();
    syncProxyControls();
});
        ...
        function clearResultsAndCounts() {
            ...
            if (remainingCreditsDisplay) {
                remainingCreditsDisplay.textContent = '--';
            }
        }
        ...
        async function processCard(card) {
            ...
            try {
                let proxyParams = { query: '', proxy: null };
                try {
                    proxyParams = determineProxyParams();
                } catch (proxyError) {
                    if (proxyError.message === 'NO_SAVED_PROXIES') {
                        throw new Error('No saved proxies available. Disable saved proxy rotation or refresh proxies.');
                    }
                    throw proxyError;
                }

                const response = await fetch(`check_card_ajax.php?cc=${encodeURIComponent(card)}&site=${encodeURIComponent(currentSite)}${proxyParams.query}`);
                if (!response.ok) {
                ...
                if (typeof data.remaining_credits !== 'undefined') {
                    updateRemainingCreditsDisplayValue(data.remaining_credits);
                }
        ...
        async function processBatch(cards, batchSize = 10) {
            ...
            for (const chunk of chunks) {
                ...
                const promises = chunk.map(card => {
                    ...
                    return fetch(`check_card_ajax.php?cc=${encodeURIComponent(card)}&site=${encodeURIComponent(currentSite)}${proxyParams.query}`)
                        .then(response => {
                            if (!response.ok) {
                                throw new Error(`HTTP error! status: ${response.status}`);
                            }
        ...
        document.getElementById('checkForm').addEventListener('submit', async function(event) {
            ...
            if (useSavedProxiesToggle && useSavedProxiesToggle.checked && !useMyIpCheckbox.checked && savedProxyState.proxies.length === 0) {
                alert('Saved proxy rotation is enabled but no saved proxies are available. Refresh or add proxies first.');
                return;
            }
