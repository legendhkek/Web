...
$webhook_path = '/telegram_webhook_enhanced.php';
$webhook_url = AppConfig::DOMAIN ? rtrim(AppConfig::DOMAIN, '/') . $webhook_path : $webhook_path;
$owner_ids = AppConfig::OWNER_IDS ?? [];
$owner_id = $owner_ids[0] ?? (TelegramConfig::CHAT_ID ?? null);
...
<body>
    <div class="container">
        ...
        $botInfo = performTelegramApiRequest('getMe', [], [
            'method' => 'GET',
            'timeout' => 10
        ]);
        ...
        if (!($botInfo['ok'] ?? false)) {
            ...
            $description = $botInfo['description'] ?? 'Unable to reach Telegram API';
            echo "<span class='icon'>❌</span><strong>{$description}</strong>";
            echo "</div>";
            exit;
        }
        ...
        echo "Username: <code>@" . htmlspecialchars($bot_info['username'] ?? 'unknown') . "</code><br>";
        echo "ID: <code>" . htmlspecialchars($bot_info['id'] ?? 'unknown') . "</code><br>";
        echo "Name: <code>" . htmlspecialchars($bot_info['first_name'] ?? 'unknown') . "</code>";
        echo "</div>";
        ...
        $webhook_info = performTelegramApiRequest('getWebhookInfo', [], [
            'method' => 'GET',
            'timeout' => 10
        ]);
        
        if ($webhook_info['ok'] ?? false) {
        ...
        if (strpos($webhook_url, 'https://') !== 0) {
            ...
            $result = performTelegramApiRequest('setWebhook', [
                'url' => $webhook_url,
            ...
            if ($result['ok'] ?? false) {
                ...
                echo htmlspecialchars($result['description'] ?? 'Unknown error');
                echo "</div>";
            }
        }
        ...
        if ($owner_id !== null) {
            ...
            if ($result['ok'] ?? false) {
                ...
                echo "Chat ID: <code>" . htmlspecialchars($owner_id) . "</code>";
                echo "</div>";
            } else {
                echo "<div class='status-box warning'>";
                echo "<span class='icon'>⚠️</span><strong>Could not send test message:</strong><br>";
                echo htmlspecialchars($result['description'] ?? 'Owner may need to start the bot first');
                echo "</div>";
            }
        } else {
            echo "<div class='status-box warning'>";
            echo "<span class='icon'>⚠️</span><strong>No owner or notification chat configured; skipping test message.</strong>";
            echo "</div>";
        }
        ?>
