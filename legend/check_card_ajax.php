...
$useUserProxies = isset($_GET['use_user_proxies']) && ($_GET['use_user_proxies'] === '1' || $_GET['use_user_proxies'] === 'true');
...
if (!empty($card) && !empty($site)) {
    ...
    // Auto-select proxy from user's pool if enabled
    if ($useUserProxies && empty($proxy) && !$useNoProxy) {
        $userProxy = $db->getNextRotatingProxy($telegram_id);
        if ($userProxy && isset($userProxy['proxy'])) {
            $proxy = $userProxy['proxy'];
            error_log("Auto-selected user proxy: " . substr($proxy, 0, 20) . "...");
        }
    }
