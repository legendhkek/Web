...
$useDbProxy = isset($_GET['use_db_proxy']) && ($_GET['use_db_proxy'] === '1' || $_GET['use_db_proxy'] === 'true' || $_GET['use_db_proxy'] === 'yes');

// Get proxy from database if requested
if ($useDbProxy && empty($proxy) && !$useNoProxy) {
    $dbProxy = $db->getRandomLiveProxy($telegram_id);
    if ($dbProxy) {
        $proxy = $dbProxy;
    }
}
