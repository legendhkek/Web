...
<body>
    ...
    <div class="main-wrapper">
        <div class="panel">
            ...
            <form id="checkForm">
                ...
                <div class="input-group" style="margin-top:8px; align-items:center; gap:8px; flex-direction: column; align-items: flex-start;">
                    <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                        <input type="radio" id="useMyIpCheckbox" name="proxyOption" value="none" checked />
                        <label for="useMyIpCheckbox" style="margin:0;">Use my IP (no proxy)</label>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                        <input type="radio" id="useDatabaseProxy" name="proxyOption" value="database" />
                        <label for="useDatabaseProxy" style="margin:0;">Use proxy from Proxy Manager</label>
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                        <input type="radio" id="useCustomProxy" name="proxyOption" value="custom" />
                        <label for="useCustomProxy" style="margin:0;">Use custom proxy</label>
                    </div>
                    <textarea name="proxy_input" id="proxyInput" placeholder="Enter Proxy (format: ip:port:user:pass) - Optional" rows="2" style="display: none; width: 100%; margin-top: 8px;"></textarea>
                    <a href="proxy_manager.php" style="color: var(--link-color); font-size: 12px; margin-top: 4px; text-decoration: none;">
                        <i class="fas fa-external-link-alt"></i> Manage Proxies
                    </a>
                </div>
    ...
    const useMyIpCheckbox = document.getElementById('useMyIpCheckbox');
        ...
        async function processCard(card) {
            ...
            try {
                // Get connection preference: use my IP, database proxy, or custom proxy
                const proxyOption = document.querySelector('input[name="proxyOption"]:checked').value;
                let extraParam = '';
                
                if (proxyOption === 'none') {
                    extraParam = `&noproxy=1`;
                } else if (proxyOption === 'database') {
                    // Get random live proxy from database
                    extraParam = `&use_db_proxy=1`;
                } else if (proxyOption === 'custom') {
                    const proxyInputVal = document.getElementById('proxyInput').value.trim();
                    if (proxyInputVal) {
                        const proxyParts = proxyInputVal.split(':');
                        if (proxyParts.length === 4) {
                            extraParam = `&proxy=${encodeURIComponent(proxyInputVal)}`;
                        } else {
        ...
        async function processBatch(cards, batchSize = 10) {
            ...
            for (const chunk of chunks) {
                ...
                const promises = chunk.map(card => {
                    const currentSite = sites[Math.floor(Math.random() * sites.length)];
                    const proxyOption = document.querySelector('input[name="proxyOption"]:checked')?.value || 'none';
                    let extraParam = '';
                    ...
                    if (proxyOption === 'none') {
                        ...
                        if (proxyInput) {
                            ...
                            if (proxyParts.length === 4) {
                                extraParam = `&proxy=${encodeURIComponent(proxyInput)}`;
                            }
                        }
                    }
                    ...
                    return fetch(`check_card_ajax.php?cc=${encodeURIComponent(card)}&site=${encodeURIComponent(currentSite)}${extraParam}`)
                        .then(response => {
                            if (!response.ok) {
                                throw new Error(`HTTP error! status: ${response.status}`);
                            }
