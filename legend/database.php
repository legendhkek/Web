...
class Database {
    ...
    /**
     * Determine if the database is operating in fallback (file-based) mode.
     */
    ...
    /**
     * Generic user update helper to modify arbitrary fields.
     */
    public function updateUser($telegramId, array $fields) {
        if ($this->useFallback) {
            return $this->getFallback()->updateUser($telegramId, $fields);
        }
        
        $users = $this->getCollection(DatabaseConfig::USERS_COLLECTION);
        if (!isset($fields['updated_at'])) {
            $fields['updated_at'] = new MongoDB\BSON\UTCDateTime();
        }
