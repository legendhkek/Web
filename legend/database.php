...
class Database {
    ...
    public function addProxy($userId, $proxy) {
        if ($this->useFallback) {
            // For fallback, store in a simple JSON file
            ...
            return $this->getFallback()->saveData('proxies', $proxies);
        }
        
        $proxies = $this->getCollection('proxies');
        
        // Check if proxy already exists for this user
        $existing = $proxies->findOne(['user_id' => $userId, 'proxy' => $proxy]);
        if ($existing) {
            return false;
        }
        
        $result = $proxies->insertOne([
            'user_id' => $userId,
            'proxy' => $proxy,
            'status' => 'untested',
            'response_time' => 0,
    ...
    public function getUserProxies($userId) {
        if ($this->useFallback) {
            $proxies = $this->getFallback()->loadData('proxies');
            return array_filter($proxies, function($p) use ($userId) {
                return $p['user_id'] == $userId;
            });
        }
        
        $proxies = $this->getCollection('proxies');
        return $proxies->find(['user_id' => $userId], ['sort' => ['created_at' => -1]])->toArray();
    }
    
    public function getProxyById($userId, $proxyId) {
        if ($this->useFallback) {
            $proxies = $this->getFallback()->loadData('proxies');
            foreach ($proxies as $proxy) {
                if ($proxy['_id'] == $proxyId && $proxy['user_id'] == $userId) {
                    return $proxy;
                }
            }
            return null;
    ...
    public function removeProxy($userId, $proxyId) {
        if ($this->useFallback) {
            $proxies = $this->getFallback()->loadData('proxies');
            $filtered = array_filter($proxies, function($p) use ($userId, $proxyId) {
                return !($p['_id'] == $proxyId && $p['user_id'] == $userId);
            });
            return $this->getFallback()->saveData('proxies', array_values($filtered));
        }
        
        $proxies = $this->getCollection('proxies');
        try {
            $result = $proxies->deleteOne([
                '_id' => new MongoDB\BSON\ObjectId($proxyId),
                'user_id' => $userId
            ]);
    ...
    public function updateProxyStatus($userId, $proxyId, $status, $responseTime) {
        if ($this->useFallback) {
            $proxies = $this->getFallback()->loadData('proxies');
            foreach ($proxies as &$proxy) {
                if ($proxy['_id'] == $proxyId && $proxy['user_id'] == $userId) {
            ...
            $result = $proxies->updateOne(
                ...
                [
                    '$set' => [
                        ...
                        'last_tested_at' => new MongoDB\BSON\UTCDateTime(),
                        'updated_at' => new MongoDB\BSON\UTCDateTime()
                    ]
                ]
            );
    ...
    public function getRandomLiveProxy($userId) {
        if ($this->useFallback) {
            $proxies = $this->getFallback()->loadData('proxies');
            $liveProxies = array_filter($proxies, function($p) use ($userId) {
                return $p['user_id'] == $userId && $p['status'] === 'live';
            });
            return !empty($liveProxies) ? $liveProxies[array_rand($liveProxies)] : null;
        }
        
        $proxies = $this->getCollection('proxies');
        $liveProxies = $proxies->find([
            'user_id' => $userId,
            'status' => 'live'
        ])->toArray();
    ...
    public function getNextRotatingProxy($userId) {
        // Get all live proxies and rotate through them
        if ($this->useFallback) {
            $proxies = $this->getFallback()->loadData('proxies');
            $liveProxies = array_filter($proxies, function($p) use ($userId) {
                return $p['user_id'] == $userId && $p['status'] === 'live';
            });
            $liveProxies = array_values($liveProxies);
            if (empty($liveProxies)) return null;
            
            // Simple rotation based on usage count
            usort($liveProxies, function($a, $b) {
                return ($a['usage_count'] ?? 0) - ($b['usage_count'] ?? 0);
            });
            
            return $liveProxies[0];
        }
