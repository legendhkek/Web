<?php
/**
 * One-time script to add initial Stripe Auth sites
 * Run this once to populate the database with initial sites
 */
...
// Owner ID (from AppConfig)
$ownerId = AppConfig::OWNER_IDS[0];

// List of Stripe Auth sites provided by user
$sites = [
...
echo "<pre>";
echo "Starting Stripe Auth sites import...\n\n";

foreach ($sites as $site) {
    // Clean up the site (remove quotes and whitespace)
    $site = trim($site, " \"\r\n");
    ...
    try {
        $result = $db->addStripeAuthSite($site, $ownerId);
        ...
        $failed++;
        echo "✗ Failed: {$site} - Error: {$e->getMessage()}\n";
    }
}
...
echo "Duplicates/Exists: {$duplicates}\n";
echo "Failed: {$failed}\n";
echo "Total sites: " . count($sites) . "\n";
echo "</pre>";

echo "<br><a href='stripe_auth_sites.php'>Go to Stripe Auth Sites Management</a>";
?>
