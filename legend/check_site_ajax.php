...
// Send "site check started" notification to user
try {
    if (function_exists('sendTelegramHtml') && SiteConfig::get('notify_check_started', true)) {
        $site_start_notification = (
            "🔄 <b>Site Check Started</b>\n\n" .
            "👤 <b>User ID:</b> {$telegram_id}\n" .
            "🔗 <b>Site:</b> " . htmlspecialchars($site_url) . "\n" .
            "⏳ <b>Status:</b> Validating site..."
        );
        sendTelegramHtml($site_start_notification);
    }
} catch (Exception $e) {
    error_log("Failed to send site check started notification: " . $e->getMessage());
}
