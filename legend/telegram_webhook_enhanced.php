...
function handleCheckCard($parts, $chat_id, $user_id) {
    ...
    if (count($parts) < 2) {
        sendMessage($chat_id, "❌ Usage: /check &lt;card&gt;\nExample: /check 4111111111111111|12|2025|123");
        return;
    }
    ...
    if (count($card_parts) != 4) {
        sendMessage($chat_id, "❌ Invalid card format. Use: xxxx|xx|xxxx|xxx\nExample: 4111111111111111|12|2025|123");
        return;
    }
    ...
    if (!preg_match("/^\d{16}$/", $card_num) || 
        ...
        sendMessage($chat_id, "❌ Invalid card format. Please check:\n• Card number: 16 digits\n• Month: 01-12\n• Year: 4 digits\n• CVV: 3-4 digits");
        return;
    }
    ...
    $is_owner = in_array($user_id, $owner_ids);
    
    if (!$is_owner && $current_credits < 1) {
        sendMessage($chat_id, "❌ Insufficient credits. You need at least 1 credit to perform a check.\n💳 Your balance: {$current_credits}\n\nUse /claim &lt;code&gt; to redeem credits.");
        return;
    }
    
    // Send immediate response that checking is starting
    $masked_card = substr($card_num, 0, 4) . '****' . substr($card_num, -4);
    sendMessage($chat_id, "🔄 <b>Checking card...</b>\n\n💳 <b>Card:</b> <code>{$masked_card}</code>\n⏳ Please wait, this may take a few seconds...");
    
    // If fastcgi_finish_request is available, finish the HTTP response early
    // This allows the webhook to return immediately to Telegram while processing continues
    if (function_exists('fastcgi_finish_request')) {
        fastcgi_finish_request();
    }
    
    // Perform the check in background
    // Use a default site if none provided (you can modify this)
    $default_site = 'https://example.com'; // You may want to get this from config or user input
    ...
    // Process response
    if ($curl_error) {
        $result_msg = "❌ <b>Check Failed</b>\n\n";
        $result_msg .= "💳 <b>Card:</b> <code>{$masked_card}</code>\n";
        $result_msg .= "⚠️ <b>Error:</b> {$curl_error}\n";
        $result_msg .= "🔗 <b>Site:</b> {$default_site}\n";
        sendMessage($chat_id, $result_msg);
        return;
    }
    ...
    if ($http_code >= 400) {
        $result_msg = "❌ <b>Check Failed</b>\n\n";
        $result_msg .= "💳 <b>Card:</b> <code>{$masked_card}</code>\n";
        $result_msg .= "⚠️ <b>HTTP Error:</b> {$http_code}\n";
        $result_msg .= "🔗 <b>Site:</b> {$default_site}\n";
        sendMessage($chat_id, $result_msg);
        return;
    }
    ...
    if (!$data || json_last_error() !== JSON_ERROR_NONE) {
        $result_msg = "❌ <b>Check Failed</b>\n\n";
        $result_msg .= "💳 <b>Card:</b> <code>{$masked_card}</code>\n";
        $result_msg .= "⚠️ <b>Error:</b> Invalid API response\n";
        $result_msg .= "🔗 <b>Site:</b> {$default_site}\n";
        sendMessage($chat_id, $result_msg);
        return;
    }
    ...
    // Deduct credit
    $credit_deducted = $db->deductCredits($user_id, 1);
    if ($credit_deducted) {
        $freshUser = $db->getUserByTelegramId($user_id);
        $remaining_credits = intval($freshUser['credits'] ?? ($current_credits - 1));
    } else {
    ...
    try {
        if (method_exists($db, 'logToolUsage')) {
            $db->logToolUsage($user_id, 'card_checker', 1, 1);
        }
    } catch (Exception $e) {
    ...
    // Send result to user
    $result_msg = "{$status_emoji} <b>Check Complete</b>\n\n";
    $result_msg .= "💳 <b>Card:</b> <code>{$card_num}|{$month}|{$year}|{$cvv}</code>\n";
    $result_msg .= "🔗 <b>Site:</b> {$default_site}\n";
    $result_msg .= "📣 <b>Response:</b> {$status_message}\n";
    $result_msg .= "🟩 <b>Status:</b> {$final_status_type}\n";
    $result_msg .= "🏦 <b>Gateway:</b> {$gateway}\n";
    $result_msg .= "💵 <b>Amount:</b> \${$price}\n";
    if ($proxy_status !== 'N/A') {
        $result_msg .= "🌐 <b>Proxy:</b> {$proxy_status}\n";
        if ($proxy_ip !== 'N/A') {
            $result_msg .= "📍 <b>Proxy IP:</b> {$proxy_ip}\n";
        }
    }
    $result_msg .= "\n💳 <b>Remaining Credits:</b> {$remaining_credits}";
