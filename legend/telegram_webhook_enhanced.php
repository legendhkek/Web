...
try {
    ...
    error_log('Failed to initialize database/managers: ' . $e->getMessage());
    http_response_code(200);
    exit;
}
...
function handleCheckCard($parts, $chat_id, $user_id) {
    ...
    if (count($parts) < 2) {
        sendMessage($chat_id, "❌ Usage: /check &lt;card&gt;|&lt;site&gt;\nExample: /check 4111111111111111|12|2025|123 example.com");
        return;
    }
    ...
    if (!$is_owner && $current_credits < 1) {
        sendMessage($chat_id, "❌ Insufficient credits. You need at least 1 credit to check a card.\n💳 Your balance: {$current_credits}\n\n💡 Use /claim &lt;code&gt; to redeem credits!");
        return;
    }
    
    // Parse card and site from input
    $input = trim(implode(' ', array_slice($parts, 1)));
    
    // Try to parse card|site format
    $card = '';
    $site = '';
    
    if (strpos($input, '|') !== false) {
        // Format: card|site or card|month|year|cvv|site
        $parts_input = explode('|', $input);
        if (count($parts_input) >= 4) {
            // Assume last part is site, first 4 are card parts
            $card = implode('|', array_slice($parts_input, 0, 4));
            $site = end($parts_input);
        } else {
            sendMessage($chat_id, "❌ Invalid format. Use: /check &lt;card_number&gt;|&lt;month&gt;|&lt;year&gt;|&lt;cvv&gt;|&lt;site&gt;");
            return;
        }
    } else {
        sendMessage($chat_id, "❌ Invalid format. Use: /check &lt;card_number&gt;|&lt;month&gt;|&lt;year&gt;|&lt;cvv&gt;|&lt;site&gt;\nExample: /check 4111111111111111|12|2025|123 example.com");
    ...
    if (count($card_parts) !== 4) {
        sendMessage($chat_id, "❌ Invalid card format. Use: number|month|year|cvv");
        return;
    }
    ...
    if (empty($site) || filter_var($site, FILTER_VALIDATE_URL) === false) {
        ...
        if (filter_var($site, FILTER_VALIDATE_URL) === false) {
            sendMessage($chat_id, "❌ Invalid site URL. Please provide a valid URL.");
            return;
        }
    }
    
    // Send processing message
    sendMessage($chat_id, "⏳ Checking card... Please wait.");
    ...
    if ($response === false || !empty($curl_error)) {
        sendMessage($chat_id, "❌ Error checking card: " . ($curl_error ?: 'Connection failed'));
        return;
    }
    ...
    if (!$result) {
        sendMessage($chat_id, "❌ Invalid response from API. Please try again later.");
        return;
    }
    ...
    $masked_card = substr($card_parts[0], 0, 4) . '****' . substr($card_parts[0], -4);
    
    $msg = "💳 <b>Card Check Result</b>\n\n";
    $msg .= "🔢 <b>Card:</b> {$masked_card}\n";
    $msg .= "🌐 <b>Site:</b> {$site}\n";
    $msg .= "📊 <b>Status:</b> {$status}\n";
    $msg .= "💳 <b>Gateway:</b> {$gateway}\n";
    $msg .= "💰 <b>Price:</b> \${$price}\n";
    $msg .= "⏱️ <b>Time:</b> {$time}\n";
    
    // Status emoji
    if (strpos($status, 'CHARGED') !== false || strpos($status, 'LIVE') !== false) {
        $msg .= "\n✅ <b>Card is LIVE!</b>";
    } elseif (strpos($status, 'DECLINED') !== false || strpos($status, 'DEAD') !== false) {
        $msg .= "\n❌ <b>Card is DECLINED</b>";
    }
    
    sendMessage($chat_id, $msg);
    
    // Deduct credit if not owner
    if (!$is_owner) {
        $db->deductCredits($user_id, 1);
    }
}
...
function handleCheckSite($parts, $chat_id, $user_id) {
    ...
    if (count($parts) < 2) {
        sendMessage($chat_id, "❌ Usage: /site &lt;url&gt;\nExample: /site https://example.com");
        return;
    }
    ...
    if (!$is_owner && $current_credits < 1) {
        sendMessage($chat_id, "❌ Insufficient credits. You need at least 1 credit to check a site.\n💳 Your balance: {$current_credits}\n\n💡 Use /claim &lt;code&gt; to redeem credits!");
        return;
    }
    
    $site = trim(implode(' ', array_slice($parts, 1)));
    ...
    if (filter_var($site, FILTER_VALIDATE_URL) === false) {
        sendMessage($chat_id, "❌ Invalid URL. Please provide a valid website URL.\nExample: /site example.com or /site https://example.com");
        return;
    }
    
    // Send processing message
    sendMessage($chat_id, "⏳ Checking site... Please wait.");
    
    // Simple site check - verify if site is accessible
    $ch = curl_init();
    ...
    // Format result
    $msg = "🌐 <b>Site Check Result</b>\n\n";
    $msg .= "🔗 <b>URL:</b> {$site}\n";
    $msg .= "📊 <b>HTTP Code:</b> " . ($http_code ?: 'N/A') . "\n";
    $msg .= "⏱️ <b>Response Time:</b> {$total_time}ms\n";
    
    if ($response !== false && $http_code >= 200 && $http_code < 400) {
        $msg .= "\n✅ <b>Site is LIVE and accessible!</b>";
    } elseif ($http_code >= 400) {
        $msg .= "\n⚠️ <b>Site returned error code {$http_code}</b>";
    } else {
        $msg .= "\n❌ <b>Site is not accessible</b>";
        if ($curl_error) {
            $msg .= "\nError: {$curl_error}";
        }
    }
...
try {
    ...
    // Try to send error message to owner
    try {
        if (isOwner($user_id)) {
    ...
    // Catch fatal errors
    error_log('Telegram webhook fatal error: ' . $e->getMessage());
}
